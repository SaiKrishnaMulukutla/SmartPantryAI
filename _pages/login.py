...
def render() -> None:
    ...
    if submit:
        ...
        # Password correct — send 2FA OTP before granting access
        with st.spinner("Sending verification code…"):
            send_otp(email.strip().lower(), purpose="2fa")
        st.session_state.pending_email  = email.strip().lower()
        st.session_state.otp_purpose    = "2fa"
        st.session_state.pending_user   = {"id": user["id"], "email": user["email"],
                                           "name": user.get("name", "")}
        go("verify_otp")
